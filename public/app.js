...
function updateCounts(){
  ...
  if (!document.querySelector(".card.chat")?.classList.contains("is-disabled")) {
    ...
    await fetch("/api/progress", {
      ...
      body:JSON.stringify({
        studentName: state.name.firstName + " " + state.name.lastName,
...
startBtn.addEventListener("click", async () => {
  ...
  const classId = document.getElementById("classCode")?.value.trim();
  if (!classId) { formError.textContent = "Class code required"; return; }
